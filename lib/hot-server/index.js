...
module.exports = (compiler, opts) => {
  ...
  if (options.server) {
    log.info(`WebSocket Server Attached to ${host.server}:${port}`);
  }
  ...
  if (options.server) {
    options.webSocket = {
      host: host.server, // wss._server.address().address, // eslint-disable-line no-underscore-dangle
      port: port // wss._server.address().port // eslint-disable-line no-underscore-dangle
    };
  } else {
